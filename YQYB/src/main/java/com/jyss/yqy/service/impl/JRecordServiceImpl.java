...
public class JRecordServiceImpl implements JRecordService {
	 ...
	 */
	@Override
	public ResponseEntity insertJRecord(String uAccount, String pAccount,
			int depart) {
				...
				if (pList != null && pList.size() > 0) {
					...
					if (recordPList != null && recordPList.size() == 1) {
						...
						if (pJRecordList != null && pJRecordList.size() == 1) {
							...
							int count = recordMapper.insertJRecord(jRecord);
							if (count == 1) {
								return new ResponseEntity("true", "" + uAccount
										+ "分配市场成功！");
							}
							return new ResponseEntity("false", "添加失败！");
						}
	...
	public ResponseEntity updateJRecord(int id, String account) {
		...
		if (list != null && list.size() > 0) {
			...
			if (userList != null && userList.size() > 0) {
				List<JRecord> recordList = recordMapper
						.selectAllJRecord(account);
				if (recordList == null || recordList.size() == 0) {
					...
					if (count == 1) {
						...
						List<JRecord> pRecordList = recordMapper
								.selectJRecordByPid(record2.getuId());
						for (JRecord pRecord : pRecordList) {
							recordMapper.upJRecordById(userBean.getId(),
									pRecord.getId());
						}
	...
	public ResponseEntity deleteJRecord(int id) {
		...
		if (list != null && list.size() > 0) {
			...
			List<JRecord> recordList = recordMapper.selectJRecordByPid(record
					.getuId());
			if (recordList != null && recordList.size() > 0) {

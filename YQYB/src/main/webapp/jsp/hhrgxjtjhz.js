layui.config({
	...
	function getMonthList(){
		...
		$.ajax({
			...
			success : function(data){
				hhrgxjtjhzData = data;
				if(window.sessionStorage.getItem("addhhrgxjtjhz")){
					var addhhrgxjtjhz = window.sessionStorage.getItem("addhhrgxjtjhz");
					hhrgxjtjhzData = JSON.parse(addhhrgxjtjhz).concat(hhrgxjtjhzData);
				}
				//执行加载数据的方法
				hhrgxjtjhzList();
			}
	...
	function hhrgxjtjhzList(that) {
		//渲染数据
		function renderDate(data, curr) {
			var dataHtml = '';
			if (!that) {
				currData = hhrgxjtjhzData.concat().splice(curr * nums - nums, nums);
			} else {
		...
		laypage({
			cont: "page",
			pages: Math.ceil(hhrgxjtjhzData.length / nums),
			jump: function (obj) {
				$(".hhrgxjtjhz_content").html(renderDate(hhrgxjtjhzData, obj.curr));
				$('.hhrgxjtjhz_list thead input[type="checkbox"]').prop("checked", false);
				form.render();
			}
		})
	}

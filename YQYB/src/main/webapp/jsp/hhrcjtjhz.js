layui.config({
	...
	function getMonthList(){
		...
		$.ajax({
			...
			success : function(data){
				hhrcjtjhzData = data;
				if(window.sessionStorage.getItem("addhhrcjtjhz")){
					var addhhrcjtjhz = window.sessionStorage.getItem("addhhrcjtjhz");
					hhrcjtjhzData = JSON.parse(addhhrcjtjhz).concat(hhrcjtjhzData);
				}
				//执行加载数据的方法
				hhrcjtjhzList();
			}
	...
	function hhrcjtjhzList(that) {
		//渲染数据
		function renderDate(data, curr) {
			var dataHtml = '';
			if (!that) {
				currData = hhrcjtjhzData.concat().splice(curr * nums - nums, nums);
			} else {
		...
		laypage({
			cont: "page",
			pages: Math.ceil(hhrcjtjhzData.length / nums),
			jump: function (obj) {
				$(".hhrcjtjhz_content").html(renderDate(hhrcjtjhzData, obj.curr));
				$('.hhrcjtjhz_list thead input[type="checkbox"]').prop("checked", false);
				form.render();
			}
		})
	}

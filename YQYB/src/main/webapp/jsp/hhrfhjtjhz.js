layui.config({
	...
	function getMonthList(){
		...
		$.ajax({
			...
			success : function(data){
				hhrfhjtjhzData = data;
				if(window.sessionStorage.getItem("addhhrfhjtjhz")){
					var addhhrfhjtjhz = window.sessionStorage.getItem("addhhrfhjtjhz");
					hhrfhjtjhzData = JSON.parse(addhhrfhjtjhz).concat(hhrfhjtjhzData);
				}
				//执行加载数据的方法
				hhrfhjtjhzList();
			}
	...
	function hhrfhjtjhzList(that) {
		//渲染数据
		function renderDate(data, curr) {
			var dataHtml = '';
			if (!that) {
				currData = hhrfhjtjhzData.concat().splice(curr * nums - nums, nums);
			} else {
		...
		laypage({
			cont: "page",
			pages: Math.ceil(hhrfhjtjhzData.length / nums),
			jump: function (obj) {
				$(".hhrfhjtjhz_content").html(renderDate(hhrfhjtjhzData, obj.curr));
				$('.hhrfhjtjhz_list thead input[type="checkbox"]').prop("checked", false);
				form.render();
			}
		})
	}
